...
public class SysManagerFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response,
		...
		SysManagerModel sum = LoginUserHelper.getLoginUserModel();

		/**
		 * 如果帐号不存在或者帐号已经被冻结
		 */
		if (sum == null && 
			      (!httpRequest.getRequestURI().contains("/admin/toLogin")) && 
			      (!httpRequest.getRequestURI().contains("/admin/login"))){
			String toUri = httpRequest.getContextPath() + "/admin/toLogin";
