...
public class DateUtil {
	 ...
	 * @return
	 */
	public static String dueTime(int mon){
		
		Calendar calendar = Calendar.getInstance();
		java.util.Date date = DateUtil.toDate(getSysDateString());
	...
	/**
	 * 获取指定时间添加比如12:15  加15分钟成 12:30 
	 * @param min
	 * @return
	 */
	public static String getMinTime(String startTime, int min){
	 SimpleDateFormat sdf = new SimpleDateFormat("HH:mm");
	 Calendar nowTime = Calendar.getInstance();
	 try {
	  java.util.Date  date=sdf.parse(startTime);
	  System.out.println(sdf.format(sdf.parse(startTime)));
	...
	public static String getWeekDay(String date) {
		/*定义星期数组*/
		final String []day = new String[] {"Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"}; 
		Calendar calendar = Calendar.getInstance();
		...
		/*获取是周几，周日会返回1，周1-周6会返回2到7*/
		int dayOfWeek = calendar.get(Calendar.DAY_OF_WEEK);
		/*如果返回是1为周日，设置为6（数组中第六个），否则-2，和数组里字符串对应*/
		/*if (dayOfWeek == 1)
			dayOfWeek = 6;
		else
			dayOfWeek -= 2;
	 ...
	 * @return
	 */
	public static String getSC(String startTime,String endTime){
	 ...
	 * @return
	 */
	public static String getSpecifyWeek(String time ,int week){
	 ...
	 * @return
	 */
	public static String getSpecifyWeekPer(String time ,int week){
		/*测试日期*/
		SimpleDateFormat f = new SimpleDateFormat("yyyy-MM-dd");
        ...
        int lastSunday = week*7 -dayOfWeek + 1;
        c.set(Calendar.DAY_OF_MONTH, -lastSunday);
        System.out.println("上周日是:" + f.format(c.getTime()));
	...
	public static void main(String[] args) {
        ...
        java.util.Date date;
		try {
			date = f.parse("2016-02-01 16:00:00");
			  c.setTime(date);
		} catch (ParseException e) {
			e.printStackTrace();
		}
        int dayOfWeek = c.get(Calendar.DAY_OF_WEEK);
        int lastSunday = 2*7  + 1;
        c.set(Calendar.DAY_OF_MONTH, +lastSunday);
        System.out.println("上周日是:" + f.format(c.getTime()));
