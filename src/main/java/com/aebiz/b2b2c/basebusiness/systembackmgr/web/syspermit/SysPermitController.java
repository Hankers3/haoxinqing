package com.aebiz.b2b2c.basebusiness.systembackmgr.web.syspermit;
...
@Controller
@RequestMapping("/sysback/syspermit")
public class SysPermitController extends
		BaseController<SysPermitModel, SysPermitQueryModel> {
	private SysPermitService myService;

	@Autowired
	private SysMenuService sysMenuService;

	@Autowired
	public void setMyService(SysPermitService bs) {
		this.myService = bs;
		super.setBs(bs);
	}

	public SysPermitController() {
		super("basebusiness/systembackmgr/syspermit", "SysPermit",
				SysPermitController.class);
	}
	...
	public String toUpdate(@PathVariable("uuid") String uuid, Model model,
		...
		SysPermitModel smm = (SysPermitModel) this.bs.getByUuid(uuid);
		...
		SysPermitModel resmm = preparedSendUpdateData(smm, request);

		model.addAttribute("m", resmm);

		return "basebusiness/systembackmgr/syspermit/SysPermitUpdate";
	}

	@Override
	protected boolean checkAdd(Model model, SysPermitModel m,
			HttpServletRequest request) {
		...
		if (this.getMapErrorMsg() != null && this.getMapErrorMsg().size() > 0) {
			...
			request.setAttribute(ERROR_BACK_URL,
					"basebusiness/systembackmgr/syspermit/SysPermitAdd");
			return false;
		}
	...
	@Override
	protected boolean checkUpdate(Model model, SysPermitModel m,
			HttpServletRequest request) {
		...
		if (this.getMapErrorMsg() != null && this.getMapErrorMsg().size() > 0) {
			...
			request.setAttribute(ERROR_BACK_URL,
					"basebusiness/systembackmgr/syspermit/SysPermitUpdate");
			return false;
		}
	 ...
	 * @param request
	 */
	private void checkLastLevel(SysPermitModel m, HttpServletRequest request) {
		// 判断所选菜单是否最后一级
		if (StringUtil.isEmpty(m.getMenuUuidLevel3())
			...
			List<SysMenuModel> smmList = sysMenuService.getSubMenus(m
					.getMenuUuidLevel3());
	...
	private void sendMenusToList(Model model) {
		List<SysMenuModel> returnMenus = sysMenuService.getTopMenus();
		model.addAttribute("topMenus", returnMenus);
	}

	private SysPermitModel preparedSendUpdateData(SysPermitModel smm,
			HttpServletRequest request) {
		if (smm != null) {

			List<SysMenuModel> smmList = sysMenuService.getAll();

			// 循环查询到一级菜单，将一级菜单返回到页面,不管是有没有父菜单，一级分类都发送到页面供选择
			List<SysMenuModel> level1List = new ArrayList<SysMenuModel>();
			for (SysMenuModel menu1 : smmList) {
				if (menu1 != null
			...
			String parentUuids = sysMenuService.getParentUuids(smm
					.getBelongToMenuUuid());

			System.out.println("parentUuids ===================" + parentUuids);
			...
			if (length >= 4) {
				...
				List<SysMenuModel> level4List = new ArrayList<SysMenuModel>();
				for (SysMenuModel menu : smmList) {
	 ...
	 * @return
	 */
	private void preparedAddSendUpdateData(Model model, SysPermitModel smm) {
		if (smm != null) {
