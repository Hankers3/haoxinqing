...
@Service
@Transactional
public class ProductImageCategoryServiceImpl extends BaseServiceImpl<ProductImageCategoryModel,ProductImageCategoryQueryModel> implements ProductImageCategoryService {
	...
	public List<TreeElement> getRootList(String parenUuid){
		...
		List<ProductImageCategoryModel> list =this.getSubProductImageCategoryByParentUuid(parenUuid);
		...
		for(int i=0;i<list.size();i++){
			...
			List<ProductImageCategoryModel> sublist =this.getSubProductImageCategoryByParentUuid(model.getUuid());
