...
@Service
@Transactional
public class PlatAdImageRelServiceImpl extends BaseServiceImpl<PlatAdImageRelModel,PlatAdImageRelQueryModel> implements PlatAdImageRelService {
	...
	public String create(PlatAdImageRelModel m) {
		...
		if(relUuids != null && relUuids.size() > 0){
			...
			System.out.println("=========PlatAdImageRelModel=============");
		}
		this.updateCc(m.getAdUuid());
		
		String[] imageUuids = m.getUuids();
		String[] urls = m.getUrls();
		
		String[] imageNotes = m.getImageNotes();
		
		if(imageUuids != null && imageUuids.length >0 && urls != null){
			for (int i = 0; i < imageUuids.length; i++) {
				...
				model.setOpeTime(DateFormatHelper.getNowTimeStr());
				 ...
				 String imageNote = imageNotes[i];
				 model.setImageNote(imageNote);
