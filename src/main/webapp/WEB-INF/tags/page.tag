...
<div class="y_page">
	<ul class="pagination">
		...
		<c:if test="${wm.nowPage > 1 }">
			<li class="y_pgbt"><a href="${pageContext.servletContext.contextPath}/${listPath}/${wm.nowPage-1}/${wm.pageShow}" title="上一页">&lt;</a></li>
		</c:if>
		<c:if test="${wm.nowPage <= 1}">
		...
		<c:choose>
			<c:when test="${wm.totalPage <=5}">
				<c:forEach begin="1" end="${wm.totalPage }" var="i">
					<li <c:if test="${wm.nowPage == i}">class="active"</c:if>><a href="${pageContext.servletContext.contextPath}/${listPath}/${i }/${wm.pageShow}" title="第${i}页">${i}</a></li>	
				</c:forEach>
			</c:when>
	  	...
	  	<c:if test="${wm.nowPage < wm.totalPage }">
	  		<li class="y_pgbt"><a href="${pageContext.servletContext.contextPath}/${listPath}/${wm.nowPage+1}/${wm.pageShow}" title="下一页">&gt;</a></li>
	  	</c:if>
...
<script type="text/javascript">
	function goPage() {
		...
		window.location.href="${pageContext.servletContext.contextPath}/${listPath}/"+parseInt(page)+"/${wm.pageShow}"
	}
</script>
