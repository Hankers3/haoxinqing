...
<div class="turn_page mt_10" id="page">	
	<ul class="pagination pagination-custom">
		...
		<c:if test="${wm.totalPage > 1 }">
			<li><a href="javascript:loadPageProducts('1','${wm.pageShow}')" >首页</a></li>
		</c:if>
		
		<c:if test="${wm.nowPage > 1 }">
			<li><a href="javascript:loadPageProducts('${wm.nowPage-1}','${wm.pageShow}')" title="上一页">上一页</a></li>
		</c:if>
		...
		<c:choose>
			<c:when test="${wm.totalPage <=5}">
				<c:forEach begin="1" end="${wm.totalPage }" var="i">
					<li <c:if test="${wm.nowPage == i}">class='active'</c:if>><a href="javascript:loadPageProducts('${i}','${wm.pageShow}')">${i}</a></li>	
				</c:forEach>
			</c:when>
	  ...
	  <c:if test="${wm.nowPage < wm.totalPage }">
	  	<li><a href="javascript:loadPageProducts('${wm.totalPage}','${wm.pageShow}')" title="下一页">下一页</a></li>
	  </c:if>
	  ...
	  <c:if test="${wm.totalPage <= 1 }">
		...
		<c:if test="${wm.totalPage > 1 }">
			<li><a href="javascript:loadPageProducts('${wm.totalPage}','${wm.pageShow}')" >尾页</a></li>
		</c:if>
	</ul>
	...
	<script>
		function skipPage(){
			...
			loadPageProducts(skipPage,'${wm.pageShow}');
		}
	</script>
</div>
