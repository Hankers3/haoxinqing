...
<div class="turn_page fr">
	<ul class="pagination pagination-custom">
		
		<c:if test="${wm.nowPage > 1 }">
			<li class="y_pgbt"><a href="javascript:sendMessagePage('fx','${wm.nowPage-1}','${wm.pageShow}')" title="上一页">&lt;</a></li>
		</c:if>
		<c:if test="${wm.nowPage <= 1}">
		...
		<c:choose>
			<c:when test="${wm.totalPage <=5}">
				<c:forEach begin="1" end="${wm.totalPage }" var="i">
					<li <c:if test="${wm.nowPage == i}">class="active"</c:if>><a href="javascript:sendMessagePage('fx','${i}','${wm.pageShow}')" title="第2页">${i}</a></li>	
				</c:forEach>
			</c:when>
	  	...
	  	<c:if test="${wm.nowPage < wm.totalPage }">
	  		<li class="y_pgbt"><a href="javascript:sendMessagePage('fx','${wm.totalPage}','${wm.pageShow}')" title="下一页">&gt;</a></li>
	  	</c:if>
	...
	<span class="y_pglength">共${wm.totalPage}页</span>，到第<input type="text" class="form-control" value="" id="goPage">页
...
<script type="text/javascript">
	function goPage() {
		...
		sendMessagePage('fx',parseInt(page),'${wm.pageShow}');
	}
</script>
