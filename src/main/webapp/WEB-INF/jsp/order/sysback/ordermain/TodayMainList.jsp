...
<div class="container-fluid">
	<div class="page-header">
		<div class="pull-left">
			<c:if test="${qm.orderState=='4'}"><h1>今日订单任务</h1></c:if>
			<c:if test="${qm.orderState=='7'}"><h1>动态订单任务</h1></c:if>		
		</div>
	</div>
			...
			<c:choose>
				...
				<tr>
					...
					<td>
					 ...
					 <td>
					 <c:if test="${orderMain.state=='4'}">等待接通</c:if>
					 <c:if test="${orderMain.state=='7'}">接通失败</c:if>
					</td>		
					
					<td class="y_odbtnbx">
						<a href="${pageContext.servletContext.contextPath}/sysback/order/viewTask/${orderMain.uuid}" class='btn' rel='tooltip' title='查看'>查看</a>
