...
<body>
	<div class="container-fluid">
		...
		<div class="box box-bordered bordered-top">
			...
			<form id="mainForm" action="${pageContext.servletContext.contextPath}/sysback/orderrevisit/add" commandName="m" method="post" class='form-horizontal form-validate form-bordered' >
					<input type="hidden" name="orderMainUuid" value="${m.uuid}"/>
					<input type="hidden" name="customerUuid" value="${customer.uuid}"/>
					<div class="form-group">
						<div class="col-sm-12">
							<i class="fa fa-user"></i>回访详情
						</div>
					</div>
			      ...
			      <tbody>
							<tr>
								<th width="15%">用户满意度</th>
								<td width="85%"><input type="radio" name="revistScore" value="1"  checked="checked" />很满意
									<input type="radio" name="revistScore" value="2" />满意
									<input type="radio" name="revistScore" value="3" />一般 
									<input type="radio" name="revistScore" value="4" />不满意 
								</td>
							</tr>
							<tr>	
								<th>用户意见调查</th>
				...
				<div class="y_fixbtnbox">
					<div class="y_fixedbtn">		
							<button type="submit" id="submit1" class="btn btn-primary btn-large a_size_1"  >提交</button>
							<c:if test="${m.state=='5'}">
...
function orderPass(orderUuid,state){
		var url="${pageContext.servletContext.contextPath}/sysback/orderrevisit/checkOrder";
		$.post(url,{
				"note":"",
				"orderId":orderUuid,
				"state":state,
			...
			function(data){
				if(data=="success"){
					alert("转动态任务成功");
					window.location.href="${pageContext.servletContext.contextPath}/sysback/order/list/5/1/20?init=true";
				}
			}
		);
}
